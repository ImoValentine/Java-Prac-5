...
public class SudokuBoard
{
    ...
    {   // Removes a digit dgt from each element of a string array
		String[] aArr = new String[arr.length];
		for (int indx=0; indx<arr.length; indx++){
            aArr[indx] = this.removeDigitFromStringOfLength2orMore(dgt, arr[indx]);
		}
        return aArr;
    }
    ...
    {   // Removes a digit dgt from ALL strings of length 2 or more in a row
        ...
        String digt = board[r][c];
	 	String[] row = getRow(r);
		String[] temp = removeDigitFromArray(digt, row);
		setRow(r, temp);	
    }
    ...
    {   // Removes a digit dgt from ALL strings of length 2 or more in a 3x3 region
		String digt = board[row][col];
		String[] miniSquare = get3x3region(row, col);
		String[] temp = removeDigitFromArray(digt, miniSquare);
		set3x3region(row, col, temp);   
    }
    ...
    {	// joins all strings in a column and returns a single string
		...
		String temp = joinAllStringsInArray(col);
		return temp;
    }

	public String joinAllStringsIn3x3region(int r, int c)
    {	// joins all strings in a 3x3 region and returns a single string
		String[] miniSquare = get3x3region(r, c);
		String temp = joinAllStringsInArray(miniSquare);
		return temp;
    }
	...
	public void identifyHiddenSinglesInSquare(int r, int c)
	{
		...
		{	String digit = extractDigitAtPos(pos, strngOfNumbrs);
            if(onlyNumberInRowColOr3x3region(digit, r, c)){
